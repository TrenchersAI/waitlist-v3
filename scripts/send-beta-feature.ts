// Third send: the product-change announcement, to people who have already signed in.
//
...
//
//   pnpm exec tsx scripts/send-beta-feature.ts
//   pnpm exec tsx scripts/send-beta-feature.ts --send --minutes 30
...
async function reputation() {
  ...
  const [sent, bounced, complained] = await Promise.all([
    p.betaInvite.count({ where: { campaign: BETA_CAMPAIGN, featureSentAt: { not: null } } }),
    p.betaInvite.count({ where: { campaign: BETA_CAMPAIGN, featureBouncedAt: { not: null } } }),
    p.betaInvite.count({ where: { campaign: BETA_CAMPAIGN, featureComplainedAt: { not: null } } }),
  ]);
  return {
...
async function main() {
  ...
  // EVERYONE still reachable, not just signed-in users. This is a product
  // announcement rather than an activation nudge, so it is equally relevant
  // to someone trading daily and to someone who has not been back yet.
  // signedIn is still computed, purely so the log states the split.
  ...
  console.log(`\nBeta product-change announcement - ${new Date().toISOString()}`);
  ...
  console.log(`Audience (all reachable): ${pending.length}`);
  console.log(`  of whom signed in: ${signedIn.size}`);
  ...
  for (let i = 0; i < pending.length; i += batch) {
    ...
    const payload = await Promise.all(chunk.map(async (row) => {
      ...
      return {
        from: fromEmail!, to: row.subscriber.email, subject: BETA_FEATURE_SUBJECT,
        html: await buildBetaFeatureHtml(copy), text: buildBetaFeatureText(copy), replyTo,
        headers: {
